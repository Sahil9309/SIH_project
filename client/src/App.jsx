...
function App() {
  ...
  const handleQuery = async (e) => {
    ...
    try {
      ...
      setMessages([...newMessages, { sender: 'bot', text: 'Sorry, an error occurred while getting a response.' }]);
    } finally {
      setIsQuerying(false);
    }
  };
      ...
      <div className="relative min-h-screen flex flex-col items-center px-4 py-8 sm:px-6 lg:px-8">
        <div className="w-full max-w-5xl">
          <header className="mb-10 text-center">
            ...
            <p className="text-slate-400 text-lg">Query your documents in any language</p>
          </header>
                ...
                <div className="space-y-4">
                  ...
                  {files.length > 0 && (
                    <div className="bg-slate-900/50 rounded-xl p-4 border border-slate-700/30">
                      <p className="text-sm text-slate-400 mb-2">{files.length} file(s) selected</p>
                  ...
                  {isReady && (
                    <div className="flex items-center gap-2 p-3 bg-emerald-500/10 border border-emerald-500/20 rounded-xl animate-in fade-in duration-500">
                      ...
                      <p className="text-sm text-emerald-400">Ready to answer questions</p>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="mt-6 bg-slate-800/30 backdrop-blur-sm border border-slate-700/30 rounded-xl p-5">
                ...
                <div className="flex flex-wrap gap-2">
                  {['PDF', 'PNG', 'JPG', 'MP3', 'WAV', 'M4A'].map(format => (
                    <span key={format} className="px-3 py-1 bg-slate-700/50 text-slate-400 text-xs rounded-lg">
                      {format}
                    </span>
                  ))}
                </div>
                    ...
                    <div className="flex items-center justify-center h-full">
                      <div className="text-center">
                        ...
                        <p className="text-slate-500 text-sm">
                          {isReady ? "Start by asking a question below" : "Upload your documents to begin"}
                        </p>
                      </div>
                    </div>
                  )}
